...
export function ShadcnCloneRouteViewer({ surface, slug }: ShadcnCloneRouteViewerProps) {
  ...
  const oppositeSurface: CloneSurface = surface === "desktop" ? "web" : "desktop";
  ...
  return (
    <div
      className={cn(
        ...
        surface === "desktop" ? "h-[calc(100vh-var(--window-titlebar-height,0px))]" : "h-screen"
      )}
    >
      <aside className="hidden w-[360px] shrink-0 border-r border-border bg-muted/20 lg:flex lg:flex-col">
        <div className="space-y-3 border-b border-border p-3">
          <div className="flex items-center justify-between gap-2">
            <p className="text-sm font-semibold">{t("shadcnCloneSkill.routeViewer.sidebarTitle")}</p>
            ...
            {groupedEntries.map((grouped) => (
              <div key={grouped.group} className="space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-muted-foreground">{grouped.entries.length}</span>
                </div>
                ...
                {grouped.entries.map((entry) => {
                  ...
                  return (
                    ...
                    >
                      <Link href={getCloneRouteHrefForEntry(surface, entry)} prefetch={false}>
                        <span className="truncate font-mono text-xs">{entry.sourcePath}</span>
                      </Link>
                    </Button>
                  );
                })}
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="border-b border-border">
          <div className="flex h-14 items-center gap-2 px-3 md:px-4">
            <p className="truncate font-mono text-xs text-muted-foreground">{activeEntry.sourcePath}</p>
        ...
        <main className="min-h-0 flex-1 p-3 md:p-4">
          <Card className="flex h-full min-h-0 flex-col overflow-hidden">
            <CardHeader className="border-b border-border py-3">
              <CardTitle className="text-base">{t("shadcnCloneSkill.routeViewer.previewTitle")}</CardTitle>
              <CardDescription>
                {t("shadcnCloneSkill.routeViewer.previewDescription", { path: activeEntry.sourcePath })}
              </CardDescription>
            </CardHeader>
