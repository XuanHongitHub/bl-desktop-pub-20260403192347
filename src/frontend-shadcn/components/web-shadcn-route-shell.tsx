...
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/frontend-shadcn/ui/card";
import { Input } from "@/frontend-shadcn/ui/input";
import { ScrollArea } from "@/frontend-shadcn/ui/scroll-area";
import { cn } from "@/lib/utils";

type WebRouteGroup = "marketing" | "legal" | "account" | "checkout" | "admin" | "lab";
...
const WEB_ROUTES: WebRouteDefinition[] = [
  ...
  { path: "/legal/privacy", group: "legal", labelKey: "portalSite.footer.privacy" },
  { path: "/legal/refund", group: "legal", labelKey: "portalSite.footer.refund" },
  { path: "/account", group: "account", labelKey: "portalSite.nav.dashboard" },
  { path: "/account/billing", group: "account", labelKey: "portalSite.nav.billing" },
...
export function WebShadcnRouteShell({ routePath }: WebShadcnRouteShellProps) {
  ...
  const groupedRoutes = useMemo(() => {
    const groups: WebRouteGroup[] = ["marketing", "legal", "account", "checkout", "admin", "lab"];
    return groups
      .map((group) => ({
        group,
    ...
    <div className="min-h-screen bg-background text-foreground">
      <header className="border-b border-border bg-background/95 backdrop-blur">
        <div className="mx-auto flex h-14 w-full max-w-[1400px] items-center gap-2 px-4">
          <Link href="/" className="inline-flex items-center gap-2 text-sm font-semibold" prefetch={false}>
        ...
        <aside className="space-y-4">
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-base">{t("webShadcn.routesTitle")}</CardTitle>
              <CardDescription>{t("webShadcn.routesDescription")}</CardDescription>
            </CardHeader>
                    ...
                    return (
                      ...
                      >
                        <Link href={item.path} prefetch={false}>
                          <span className="truncate">{item.labelKey ? t(item.labelKey) : toPathLabel(item.path)}</span>
                        </Link>
                      </Button>
                    );
                  })}
          ...
          <div className="space-y-4 pr-2">
            <Card>
              ...
              <CardContent className="grid gap-3 sm:grid-cols-2">
                <div className="rounded-lg border border-border bg-card p-3">
                  <p className="mb-1 text-xs text-muted-foreground">{t("webShadcn.currentRouteLabel")}</p>
                  <p className="font-mono text-sm">{activeRoute.path}</p>
                </div>
                <div className="rounded-lg border border-border bg-card p-3">
                  <p className="mb-1 text-xs text-muted-foreground">{t("webShadcn.groupLabel")}</p>
                  <p className="text-sm font-medium">{t(`webShadcn.groups.${activeRoute.group}`)}</p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>{t("webShadcn.componentDescription")}</CardDescription>
              </CardHeader>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>{t("webShadcn.nextDescription")}</CardDescription>
              </CardHeader>
              <CardContent className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                <Button asChild variant="outline" className={cn("justify-between")}>
