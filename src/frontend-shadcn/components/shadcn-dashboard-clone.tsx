...
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/frontend-shadcn/ui/select";
...
type MainSection = "docs" | "components" | "blocks" | "charts" | "directory" | "create";
type ExampleSection = "dashboard" | "tasks" | "playground" | "authentication" | "rtl";
...
const mainNav: MainSection[] = ["docs", "components", "blocks", "charts", "directory", "create"];
...
const sidebarTopKeys = ["dashboard", "lifecycle", "analytics", "projects", "team"] as const;
const sidebarBottomKeys = ["dataLibrary", "reports", "wordAssistant", "more"] as const;
...
function SidebarMenuPanel({ t }: { t: ReturnType<typeof useTranslation>["t"] }) {
  return (
    <aside className="border-r border-border bg-muted/20 p-3">
      <div className="mb-3 flex items-center gap-2 px-2 py-1">
        ...
        <span className="text-lg font-semibold">{t("shadcnCloneSkill.sidebar.orgName")}</span>
      </div>

      <div className="space-y-1 px-2 py-2">
        <p className="text-xs text-muted-foreground">{t("shadcnCloneSkill.sidebar.home")}</p>
      ...
      <div className="space-y-1 px-2 py-2">
        <p className="text-xs text-muted-foreground">{t("shadcnCloneSkill.sidebar.documents")}</p>
        {sidebarBottomKeys.map((key) => (
          <div key={key} className="flex items-center gap-2 rounded-md px-2 py-1.5 text-sm">
            {key === "more" ? <MoreHorizontal className="h-4 w-4" /> : <FileText className="h-4 w-4" />}
  ...
  return (
    <main className="space-y-4 p-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold">{t("shadcnCloneSkill.content.title")}</h1>
        <Button size="sm">
        ...
        {stats.map((item) => (
          <Card key={item.key}>
            <CardHeader className="space-y-2 pb-2">
              <div className="flex items-center justify-between">
                <CardDescription>{t(`shadcnCloneSkill.stats.${item.key}.title`)}</CardDescription>
                <Badge variant="secondary">{item.trend}</Badge>
              </div>
              <CardTitle className="text-4xl tracking-tight">{item.value}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-1 pt-0">
              <p className="text-sm font-medium">{t(`shadcnCloneSkill.stats.${item.key}.description`)}</p>
              <p className="text-sm text-muted-foreground">{t(`shadcnCloneSkill.stats.${item.key}.subDescription`)}</p>
            </CardContent>
          </Card>
        ))}
      </div>
      ...
      <Card>
        <CardHeader className="flex flex-row items-start justify-between gap-3">
          <div>
            ...
            <CardDescription>{t("shadcnCloneSkill.chart.description")}</CardDescription>
          </div>
        ...
        <CardContent>
          <ChartContainer config={visitorChartConfig} className="h-[280px] w-full">
            <AreaChart data={visitorChartData} margin={{ left: 8, right: 8, top: 12, bottom: 0 }}>
              <CartesianGrid vertical={false} />
              <XAxis dataKey="month" tickLine={false} axisLine={false} tickMargin={8} />
              <ChartTooltip cursor={false} content={(props) => <ChartTooltipContent {...(props as any)} hideLabel />} />
              <Area
                dataKey="visitors"
                type="natural"
...
function TasksPanel({ t }: { t: ReturnType<typeof useTranslation>["t"] }) {
  return (
    <main className="space-y-4 p-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold">{t("shadcnCloneSkill.pages.tasks.title")}</h1>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>{t("shadcnCloneSkill.pages.tasks.tableDescription")}</CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead>{t("shadcnCloneSkill.pages.tasks.columns.task")}</TableHead>
                <TableHead>{t("shadcnCloneSkill.pages.tasks.columns.status")}</TableHead>
                <TableHead>{t("shadcnCloneSkill.pages.tasks.columns.priority")}</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {fakeTasks.map((task) => (
...
function PlaygroundPanel({ t }: { t: ReturnType<typeof useTranslation>["t"] }) {
  return (
    <main className="space-y-4 p-4">
      <h1 className="text-2xl font-semibold">{t("shadcnCloneSkill.pages.playground.title")}</h1>
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle>{t("shadcnCloneSkill.pages.playground.controls")}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid gap-2">
              <Label htmlFor="clone-search">{t("shadcnCloneSkill.pages.playground.searchLabel")}</Label>
              <Input id="clone-search" placeholder={t("shadcnCloneSkill.pages.playground.searchPlaceholder")} />
            </div>
            ...
            <div className="flex items-center gap-2 rounded-md border border-border p-3">
              ...
              <Label htmlFor="clone-check">{t("shadcnCloneSkill.pages.playground.includeAssets")}</Label>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle>{t("shadcnCloneSkill.pages.playground.preview")}</CardTitle>
            <CardDescription>{t("shadcnCloneSkill.pages.playground.previewDescription")}</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            {componentNames.slice(0, 9).map((name) => (
              <div key={name} className="rounded-md border border-border px-3 py-2 text-sm">
                {name}
              </div>
            ))}
          </CardContent>
...
function AuthenticationPanel({ t }: { t: ReturnType<typeof useTranslation>["t"] }) {
  return (
    <main className="grid gap-4 p-4 lg:grid-cols-2">
      <Card>
        <CardHeader>
          <CardTitle>{t("shadcnCloneSkill.pages.authentication.signInTitle")}</CardTitle>
          <CardDescription>{t("shadcnCloneSkill.pages.authentication.signInDescription")}</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="grid gap-2">
            <Label htmlFor="signin-email">{t("shadcnCloneSkill.pages.authentication.email")}</Label>
            <Input id="signin-email" placeholder="name@example.com" />
          </div>
          <div className="grid gap-2">
            <Label htmlFor="signin-password">{t("shadcnCloneSkill.pages.authentication.password")}</Label>
            <Input id="signin-password" type="password" placeholder="********" />
          </div>
          <Button className="w-full">{t("shadcnCloneSkill.pages.authentication.signInButton")}</Button>
        </CardContent>
      </Card>

      <Card>
        <CardHeader>
          <CardTitle>{t("shadcnCloneSkill.pages.authentication.signUpTitle")}</CardTitle>
          <CardDescription>{t("shadcnCloneSkill.pages.authentication.signUpDescription")}</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="grid gap-2">
            <Label htmlFor="signup-name">{t("shadcnCloneSkill.pages.authentication.name")}</Label>
            <Input id="signup-name" placeholder={t("shadcnCloneSkill.pages.authentication.namePlaceholder")} />
          </div>
          <div className="grid gap-2">
            <Label htmlFor="signup-email">{t("shadcnCloneSkill.pages.authentication.email")}</Label>
...
function RtlPanel({ t }: { t: ReturnType<typeof useTranslation>["t"] }) {
  return (
    <div dir="rtl">
      <main className="space-y-4 p-4">
        <h1 className="text-2xl font-semibold">{t("shadcnCloneSkill.pages.rtl.title")}</h1>
        <Card>
          <CardHeader>
            <CardTitle>{t("shadcnCloneSkill.pages.rtl.cardTitle")}</CardTitle>
            <CardDescription>{t("shadcnCloneSkill.pages.rtl.cardDescription")}</CardDescription>
          </CardHeader>
          <CardContent>
            <p className="text-sm text-muted-foreground">{t("shadcnCloneSkill.pages.rtl.body")}</p>
          </CardContent>
        </Card>
      </main>
    </div>
...
function ComponentsSection({ t }: { t: ReturnType<typeof useTranslation>["t"] }) {
  return (
    <main className="space-y-4 p-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold">{t("shadcnCloneSkill.components.title")}</h1>
          <p className="text-sm text-muted-foreground">{t("shadcnCloneSkill.components.description")}</p>
        </div>
        <Badge variant="secondary">{componentNames.length}</Badge>
      </div>
      <div className="grid gap-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {componentNames.map((name) => (
          <div key={name} className="rounded-md border border-border bg-card px-3 py-2 text-sm">
            {name}
          </div>
        ))}
      </div>
...
function ChartsSection({ t }: { t: ReturnType<typeof useTranslation>["t"] }) {
  return (
    <main className="space-y-4 p-4">
      <h1 className="text-2xl font-semibold">{t("shadcnCloneSkill.charts.title")}</h1>
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader>
          ...
          <CardContent>
            <ChartContainer config={visitorChartConfig} className="h-[260px] w-full">
              <AreaChart data={visitorChartData}>
                ...
                <ChartTooltip content={(props) => <ChartTooltipContent {...(props as any)} />} />
                <Area dataKey="visitors" type="natural" fill="var(--color-visitors)" fillOpacity={0.25} stroke="var(--color-visitors)" />
              </AreaChart>
            </ChartContainer>
          </CardContent>
        </Card>
...
function DirectorySection({ t }: { t: ReturnType<typeof useTranslation>["t"] }) {
  ...
  const rows = useMemo(
    () =>
      urls.map((url, index) => ({
        index: index + 1,
        url,
        category: Object.entries(grouped).find(([, list]) => list.includes(url))?.[0] ?? "other",
      })),
    [urls, grouped]
  );
  ...
  return (
    <main className="space-y-4 p-4">
      <h1 className="text-2xl font-semibold">{t("shadcnCloneSkill.directory.title")}</h1>
        ...
        <CardContent className="pt-6">
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead>#</TableHead>
                <TableHead>{t("shadcnCloneSkill.directory.category")}</TableHead>
              ...
              <TableBody>
                {rows.map((row) => (
                  <TableRow key={row.url}>
                    ...
                    <TableCell className="font-mono text-xs text-muted-foreground">{row.url}</TableCell>
                  </TableRow>
                ))}
            </TableBody>
          </Table>
...
function CreateSection({ t }: { t: ReturnType<typeof useTranslation>["t"] }) {
  return (
    <main className="space-y-4 p-4">
      <h1 className="text-2xl font-semibold">{t("shadcnCloneSkill.create.title")}</h1>
      <Card>
        <CardHeader>
          <CardTitle>{t("shadcnCloneSkill.create.cardTitle")}</CardTitle>
          <CardDescription>{t("shadcnCloneSkill.create.cardDescription")}</CardDescription>
        </CardHeader>
...
function DocsSection({ t }: { t: ReturnType<typeof useTranslation>["t"] }) {
  return (
    <main className="space-y-4 p-4">
      <h1 className="text-2xl font-semibold">{t("shadcnCloneSkill.docs.title")}</h1>
      <Card>
        <CardHeader>
          <CardTitle>{t("shadcnCloneSkill.docs.cardTitle")}</CardTitle>
          <CardDescription>{t("shadcnCloneSkill.docs.cardDescription")}</CardDescription>
        </CardHeader>
...
export function ShadcnDashboardClone({ surface }: ShadcnDashboardCloneProps) {
  ...
  const [activeExample, setActiveExample] = useState<ExampleSection>("dashboard");
    ...
    <DirectionProvider direction="ltr">
      <div
        className={cn(
          ...
          surface === "desktop" ? "h-[calc(100vh-var(--window-titlebar-height,0px))]" : "min-h-screen"
        )}
      >
          ...
          <div className="mx-auto flex h-16 w-full max-w-[1320px] items-center gap-4 px-4">
              ...
              <nav className="hidden items-center gap-1 md:flex">
                {mainNav.map((key) => (
                  <Button
                    ...
                    variant="ghost"
                    size="sm"
                    className={cn("text-sm font-medium", activeMain === key ? "bg-accent" : "")}
            ...
            <div className="ml-auto flex items-center gap-2">
              <div className="hidden w-[300px] md:block">
                <Input placeholder={t("shadcnCloneSkill.header.searchPlaceholder")} />
              </div>
        ...
        <div className="mx-auto flex w-full max-w-[1320px] items-center justify-between px-4 py-3">
          {activeMain === "blocks" ? (
            <Tabs value={activeExample} onValueChange={(v) => setActiveExample(v as ExampleSection)}>
              <TabsList className="h-10">
                <TabsTrigger value="dashboard">{t("shadcnCloneSkill.header.subNav.dashboard")}</TabsTrigger>
                <TabsTrigger value="tasks">{t("shadcnCloneSkill.header.subNav.tasks")}</TabsTrigger>
                <TabsTrigger value="playground">{t("shadcnCloneSkill.header.subNav.playground")}</TabsTrigger>
                <TabsTrigger value="authentication">{t("shadcnCloneSkill.header.subNav.authentication")}</TabsTrigger>
            ...
            <p className="text-sm text-muted-foreground">{t("shadcnCloneSkill.header.sectionHint")}</p>
          )}
          ...
          <div className="hidden items-center gap-2 md:flex">
            <Select defaultValue="neutral">
              ...
              <SelectContent>
                <SelectItem value="neutral">{t("shadcnCloneSkill.header.neutral")}</SelectItem>
              </SelectContent>
            </Select>
          ...
          {activeMain === "blocks" ? (
            <div className="grid min-h-0 w-full grid-cols-1 overflow-hidden rounded-xl border border-border md:grid-cols-[240px_1fr]">
              ...
              <ScrollArea className="min-h-0">
                {activeExample === "dashboard" ? <DashboardPanel t={t} /> : null}
                {activeExample === "tasks" ? <TasksPanel t={t} /> : null}
                {activeExample === "playground" ? <PlaygroundPanel t={t} /> : null}
                {activeExample === "authentication" ? <AuthenticationPanel t={t} /> : null}
            ...
            <div className="w-full overflow-hidden rounded-xl border border-border">
              <ScrollArea className="h-full min-h-0">
                ...
                {activeMain === "components" ? <ComponentsSection t={t} /> : null}
