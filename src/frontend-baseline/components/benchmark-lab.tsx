...
import {
  ...
  TabsList,
  TabsTrigger,
} from "@/frontend-baseline/shadcn/ui/tabs";
import type {
  BaselineProfile,
  ProfileBenchmarkRow,
...
export function BaselineBenchmarkLab() {
  ...
  const runSingle = useCallback(
    async (
      ...
      const viewport =
        mode === "legacy"
          ? legacyViewportRef.current
          : optimizedViewportRef.current;
      const { scrollFps, minFps } = await measureScrollFps(viewport);

      const patchTargets = pickPatchTargets(datasetSize);
      const patchStart = performance.now();
      if (mode === "legacy") {
        setLegacyRows((previous) =>
          applyLegacyPatchBurst(previous, patchTargets),
        );
      } else {
      ...
      const totalWindow =
        renderLatencyMs + interactionDelayMs + SCROLL_WINDOW_MS + patchDuration;
  ...
  return (
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-4 p-4 md:p-6">
      ...
      <Tabs
        value={activeMode}
        onValueChange={(value) => setActiveMode(value as BenchmarkMode)}
      >
        <TabsList>
          <TabsTrigger value="legacy">
            {t("frontendBaseline.benchmark.legacyTab")}
          </TabsTrigger>
          ...
          <Card>
            <CardHeader className="gap-2">
              <CardTitle className="text-sm">
                {t("frontendBaseline.benchmark.legacyTitle")}
              </CardTitle>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-sm">
            {t("frontendBaseline.benchmark.resultsTitle")}
          </CardTitle>
        </CardHeader>
              ...
              <thead>
                <tr className="border-b border-border text-left text-muted-foreground">
                  <th className="px-2 py-2">
                    {t("frontendBaseline.benchmark.columns.mode")}
                  </th>
                  <th className="px-2 py-2">
                    {t("frontendBaseline.benchmark.columns.size")}
                  </th>
                  <th className="px-2 py-2">
                    {t("frontendBaseline.benchmark.columns.render")}
                  </th>
                  <th className="px-2 py-2">
                    {t("frontendBaseline.benchmark.columns.interaction")}
                  </th>
                  <th className="px-2 py-2">
                    {t("frontendBaseline.benchmark.columns.scrollFps")}
                  </th>
                  <th className="px-2 py-2">
                    {t("frontendBaseline.benchmark.columns.cpu")}
                  </th>
                  <th className="px-2 py-2">
                    {t("frontendBaseline.benchmark.columns.memory")}
                  </th>
                </tr>
              </thead>
              <tbody>
