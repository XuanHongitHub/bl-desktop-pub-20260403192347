...
export default function AdminAbuseTrustPage() {
  ...
  const selectedSection = searchParams.get("section")?.trim() ?? "";
  ...
  const visibleFlaggedRows = useMemo(() => {
    if (!selectedWorkspaceId) {
      return flaggedRows;
    }
    return flaggedRows.filter((row) => row.workspaceId === selectedWorkspaceId);
  }, [flaggedRows, selectedWorkspaceId]);

  const sensitiveAuditLogs = useMemo(
    () =>
      auditLogs
        .filter((row) =>
          /membership|invite|share|entitlement|auth\.|billing\./u.test(
            row.action,
          ),
        )
        .slice(0, 20),
  ...
  useEffect(() => {
    ...
    const node = document.getElementById(selectedSection);
    if (!node) {
      return;
    }
      ...
      actions={
        <div className="flex items-center gap-2">
          <Button asChild size="sm" variant="outline">
            <Link href="/admin/abuse-trust">
              {t("portalSite.admin.nav.abuseTrust")}
            </Link>
          </Button>
          {detailOnlyMode ? (
            <Button asChild size="sm" variant="outline">
      ...
      <section className="mx-auto w-full max-w-[1180px] space-y-4 text-sm">
        <div className="grid gap-3 sm:grid-cols-3">
          ...
          <div className="rounded-xl border border-border bg-card p-4">
            ...
            <p className="mt-1 text-sm font-semibold text-foreground">
              {
                rows.filter((row) => row.entitlementState === "read_only")
                  .length
              }
            </p>
          </div>
        ...
        <div className="grid gap-4 xl:grid-cols-[minmax(0,1.2fr)_minmax(0,0.8fr)]">
          <div id="queue" className="rounded-xl border border-border bg-card">
            <div className="flex items-center justify-between border-b border-border px-4 py-3">
              <p className="text-sm font-medium text-foreground">
                {t("portalSite.admin.abuseTrust.queueTitle")}
              </p>
              <Badge variant="outline">{visibleFlaggedRows.length}</Badge>
            </div>
                    ...
                    >
                      <div className="min-w-0 space-y-1">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <Badge
                            variant={
                              row.entitlementState === "read_only"
                                ? "destructive"
                                : "outline"
                            }
                          >
                        ...
                        <p className="text-xs text-muted-foreground">
                          ...
                          {row.activeInvites} invites · {row.activeShareGrants}{" "}
                          shares
                        </p>
                      </div>
          ...
          <div id="audit" className="rounded-xl border border-border bg-card">
            <div className="flex items-center justify-between border-b border-border px-4 py-3">
              <p className="text-sm font-medium text-foreground">
