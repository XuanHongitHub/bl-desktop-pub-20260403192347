...
export default function WorkspaceBillingPage() {
  ...
  const [billing, setBilling] = useState<ControlWorkspaceBillingState | null>(
    null,
  );
  ...
  const loadData = useCallback(() => {
    ...
    getWorkspaceBillingState(connection, workspaceId)
      ...
      .catch((err) => {
        showErrorToast("Lỗi tải thông tin Billing", {
  ...
  if (!billing)
    return (
      <div className="text-center py-20 text-muted-foreground">
        Không tìm thấy thông tin Billing
      </div>
    );
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <Card className="border-border/50 shadow-sm relative overflow-hidden">
        ...
        <CardHeader className="pb-4">
          ...
          <CardDescription>
            Ghi đè thủ công Gói và Giới hạn (Force Override)
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              <p className="text-sm font-medium">Gói đăng ký</p>
              <Select
                value={formPlanId}
                onValueChange={(v) => setFormPlanId(v as any)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <div className="space-y-2">
              <p className="text-sm font-medium">Chu kỳ</p>
              <Select
                value={formBillingCycle}
                onValueChange={(v) => setFormBillingCycle(v as any)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <div className="space-y-2">
              <p className="text-sm font-medium">Limit Profiles</p>
              <Input
                type="number"
                min="1"
                value={formProfileLimit}
                onChange={(e) => setFormProfileLimit(e.target.value)}
              />
            </div>
            <div className="space-y-2">
              <p className="text-sm font-medium">Limit Members</p>
              <Input
                type="number"
                min="1"
            ...
            <div className="space-y-2 col-span-2">
              <p className="text-sm font-medium">Buộc hết hạn (Expires At)</p>
              <Input
                type="datetime-local"
                value={formExpiresAt}
              ...
              <p className="text-xs text-muted-foreground">
                Bỏ trống nếu không muốn ép hạn sử dụng.
              </p>
            </div>
          </div>
        </CardContent>
      ...
      <div className="flex flex-col gap-6">
        <Card className="border-border/50 shadow-sm">
          <CardHeader className="pb-4">
            ...
            <CardDescription>
              {billing.recentInvoices.length} hóa đơn gần nhất
            </CardDescription>
          </CardHeader>
          <CardContent>
            {billing.recentInvoices.length > 0 ? (
              <div className="space-y-3">
                {billing.recentInvoices.map((inv) => (
                  <div
                    key={inv.id}
                    className="flex justify-between items-center text-sm p-3 border rounded-md"
                  >
                    <div>
