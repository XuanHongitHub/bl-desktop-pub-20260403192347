...
function buildTargetUrl(request: NextRequest, slug?: string[]): URL {
  const targetPath =
    slug && slug.length > 0 ? `/${slug.join("/")}` : "/examples/dashboard";
...
export async function GET(
  ...
  const _contentType = rawContentType.split(";")[0]?.trim() || rawContentType;
