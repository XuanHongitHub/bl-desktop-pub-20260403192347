...
import { Shield, Plus, Key, Users, Settings2, Trash2, Edit2, CheckCircle2 } from "lucide-react";
...
export const AVAILABLE_CAPABILITIES: RoleCapability[] = [
  { id: "manage_settings", label: "Quản lý cài đặt Workspace", description: "Sửa đổi tên, thông tin và cấu hình chung", category: "general" },
  { id: "create_profile", label: "Tạo Profile & Nhóm", description: "Cho phép tạo mới các trình duyệt và nhóm", category: "profiles" },
  { id: "edit_profile", label: "Chỉnh sửa Profile", description: "Sửa tên, gắn thẻ và thay đổi cấu hình kỹ thuật", category: "profiles" },
  { id: "delete_profile", label: "Xóa Profile", description: "Cho phép xóa vĩnh viễn trình duyệt", category: "profiles" },
  { id: "manage_proxy", label: "Quản lý Proxy", description: "Mua mới, cập nhật hoặc gỡ proxy", category: "profiles" },
  { id: "manage_billing", label: "Quản lý Thanh toán", description: "Nâng cấp gói, xem hóa đơn, cấn trừ ví", category: "billing" },
  { id: "invite_members", label: "Mời & Quản lý nhân sự", description: "Gửi invite mới vào Workspace", category: "team" },
  { id: "remove_members", label: "Xóa thành viên", description: "Hủy tư cách thành viên của người khác", category: "team" },
  { id: "manage_roles", label: "Quản trị Vai trò (Roles)", description: "Tạo, sửa bộ quyền tùy chỉnh", category: "team" },
];
...
const CATEGORY_MAP: Record<PermissionCategory, { label: string; icon: React.ElementType }> = {
...
export function CustomRolesManager({ roles, onAddRole, onUpdateRole, onDeleteRole, isPlatformAdmin, viewMode = "workspace" }: CustomRolesManagerProps) {
  ...
  // Group capabilities
  const groupedCaps = AVAILABLE_CAPABILITIES.reduce((acc, cap) => {
    ...
    <div className="flex h-[calc(100vh-280px)] min-h-[500px] w-full gap-4 overflow-hidden rounded-xl border border-border/70 bg-background/50 shadow-sm backdrop-blur-sm">
      ...
      <div className="flex w-[260px] flex-col border-r border-border/70 bg-card/30">
        <div className="flex items-center justify-between border-b border-border/60 px-4 py-3">
          <span className="text-sm font-semibold text-foreground">Vai trò hệ thống</span>
          {viewMode === "superadmin" && (
            <Badge variant="default" className="scale-90 text-[10px]">Super Admin</Badge>
          )}
        </div>
            ...
            {roles.map((role) => (
              ...
              >
                <div className="flex items-center gap-2">
                  <Shield className={cn("h-4 w-4", role.isSystem ? "opacity-70" : "text-primary/70")} />
                  <span className="truncate">{role.name}</span>
                </div>
                
                {role.isSystem && (
                  <Badge variant="outline" className="scale-75 px-1 py-0 border-primary/20 text-primary">System</Badge>
                )}
              </button>
            ))}
            ...
            {isCreating ? (
              ...
              >
                ...
                <div className="flex justify-end gap-1">
                  <Button variant="ghost" size="sm" className="h-6 px-2 text-[10px]" onClick={() => setIsCreating(false)}>Hủy</Button>
                  <Button variant="default" size="sm" className="h-6 px-2 text-[10px]" onClick={handleConfirmCreate}>Tạo</Button>
                </div>
              </motion.div>
            ) : (
              <Button
          ...
          <>
            <div className="flex items-end justify-between border-b border-border/60 px-6 py-4">
              <div>
                <h2 className="text-xl font-bold tracking-tight text-foreground flex items-center gap-2">
                  {activeRole.name}
                  {activeRole.isSystem && (
                    <Badge variant="secondary" className="font-normal">Mặc định hệ thống</Badge>
                  )}
                </h2>
            ...
            <ScrollArea className="flex-1 p-6">
              <div className="mx-auto max-w-3xl space-y-8 pb-10">
                {(Object.keys(groupedCaps) as PermissionCategory[]).map((category) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="grid gap-3 sm:grid-cols-2">
                        {items.map((cap) => {
                          const isEnabled = activeRole.capabilities.includes(cap.id);
                            ...
                            >
                              ...
                              <Switch 
                                ...
                                className={cn(
                                  ...
                                  activeRole.isSystem && "cursor-not-allowed opacity-50"
                                )}
                              />
                            </div>
                          );
