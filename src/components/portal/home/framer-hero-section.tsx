...
import { ArrowRight, MousePointer2, MoreHorizontal, Play } from "lucide-react";
...
export function FramerHeroSection() {
  ...
  return (
    <section className={cn(styles.section, "relative isolate bg-background text-foreground")}> 
      <div
        className={cn(
          styles.glowTop,
      ...
      >
        <div className="mx-auto w-full">
          <div
            ref={surfaceRef}
            className="relative overflow-hidden rounded-2xl border border-border/70 bg-card/80 shadow-md backdrop-blur"
          >
            ...
            <div className="grid min-h-[420px] grid-cols-12">
              <aside className="col-span-3 border-r border-border/70 bg-background/60 p-3">
                <p className="mb-2 text-[10px] uppercase text-muted-foreground">Platform</p>
                <div className="space-y-1 text-xs">
                  <div className="rounded-md bg-muted/70 px-2 py-1.5 font-medium text-foreground">Profiles</div>
                  <div className="rounded-md px-2 py-1.5 text-muted-foreground">Automation</div>
                  <div className="rounded-md px-2 py-1.5 text-muted-foreground">Proxy Center</div>
              ...
              <main className="col-span-9 p-3">
                <div className="mb-3 flex items-center justify-between">
                  <p className="text-lg font-semibold text-foreground">Profiles</p>
                  <div className="flex items-center gap-2">
                    <Button variant="outline" size="icon" className="h-7 w-7">
                      <MoreHorizontal className="h-4 w-4" />
                    </Button>
                    <Button className="h-7 rounded-md px-3 text-[11px] font-medium">Create a new profile</Button>
                  </div>
                </div>
                ...
                <div className="mb-3 flex items-center gap-2 text-[11px]">
                  <div
                    ...
                    className="rounded-md border border-border/70 bg-muted/40 px-2 py-1"
                  >
                    All · 56
                  </div>
                  <div className="rounded-md border border-border/70 bg-background px-2 py-1 text-muted-foreground">
                    Demo-Bug... · 52
                  </div>
                  <div className="rounded-md border border-border/70 bg-background px-2 py-1 text-muted-foreground">
                    Nguyen Xu... · 0
                ...
                <div className="overflow-hidden rounded-md border border-border/70 bg-background">
                  ...
                  {rows.map((row, idx) => (
                    <div key={row.id} className="grid grid-cols-12 items-center border-b border-border/60 px-2 py-1.5 text-[11px] last:border-b-0">
                      ...
                      <div className="col-span-2">
                        <Button
                          ...
                          className={cn(
                            ...
                            idx <= 1 ? "bg-foreground text-background hover:bg-foreground/90" : "bg-muted text-foreground hover:bg-muted/80",
                            idx === 0 && pulseTick % 2 === 0 && "ring-1 ring-primary/50",
                          )}
                        >
                      ...
                      <div className="col-span-3 text-foreground">{row.name}</div>
                      <div className="col-span-2 text-muted-foreground">{row.lastLaunch}</div>
                      <div className="col-span-1 text-muted-foreground">{row.proxy}</div>
