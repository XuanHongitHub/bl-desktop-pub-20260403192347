...
export default function Home() {
  ...
  const [, setIsSidebarAccountMenuOpen] = useState(false);
  ...
  const {
    ...
    includeRunningStateSync: isProfilesSectionActive || isBugideaSectionActive,
  });
  ...
  useEffect(() => {
    if (!shouldLoadWorkspaceProfileUsage || !cloudUser || !sidebarWorkspaceId) {
      setWorkspaceProfilesUsed({});
      return;
    }

    setWorkspaceProfilesUsed((current) => {
      const nextProfilesUsed = profiles.length;
      if (current[sidebarWorkspaceId] === nextProfilesUsed) {
        return current;
      }
      return {
        ...current,
        [sidebarWorkspaceId]: nextProfilesUsed,
      };
    });
  }, [
  ...
  const [groupManagementDialogOpen, setGroupManagementDialogOpen] =
    useState(false);
  const [groupManagementInitialAction, setGroupManagementInitialAction] =
    useState<"manage" | "edit" | "delete">("manage");
  const [groupManagementInitialGroupId, setGroupManagementInitialGroupId] =
  ...
  const openGroupManagementDialog = useCallback(
    (action: "manage" | "edit" | "delete", groupId: string | null = null) => {
      setHasHydratedGroupData(true);
      setGroupManagementInitialAction(action);
      setGroupManagementInitialGroupId(groupId);
          ...
          <WorkspacePageShell
            ...
            actions={
              <ProfilesWorkspaceHeaderActions
                ...
                onGroupManagementDialogOpen={(open) => {
                  if (open) {
                    openGroupManagementDialog("manage");
                    return;
                  }
            ...
            <div className="flex min-h-0 flex-1 flex-col gap-4">
              <div className="shrink-0">
                <GroupBadges
                  ...
                  onEditGroup={(groupId) =>
                    openGroupManagementDialog("edit", groupId)
                  }
                  onDeleteGroup={(groupId) =>
                    openGroupManagementDialog("delete", groupId)
                  }
                  groups={groupsData}
                  isLoading={isLoading}
                />
  ...
  if (!cloudUser) {
    if (isCloudAuthLoading) {
      return (
        <PageLoader
          mode="fullscreen"
          className="text-[13px] leading-[1.35] tracking-[-0.005em] font-(family-name:--font-sans)"
        />
      );
    }
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background text-[13px] leading-[1.35] tracking-[-0.005em] font-(family-name:--font-sans)">
      <AppSidebar
        ...
        onAccountMenuOpenChange={setIsSidebarAccountMenuOpen}
      ...
      {groupManagementDialogOpen && (
        <GroupManagementDialog
          ...
          initialAction={groupManagementInitialAction}
          initialGroupId={groupManagementInitialGroupId}
        />
      )}
