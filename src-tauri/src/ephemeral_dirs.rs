...
fn get_ephemeral_base_dir() -> Result<PathBuf, String> {
  #[cfg(target_os = "linux")]
  {
    let base = PathBuf::from("/dev/shm/buglogin-ephemeral");
  ...
  #[cfg(not(target_os = "linux"))]
  {
    ...
    // Fallback
    let base = std::env::temp_dir().join("buglogin-ephemeral");
    std::fs::create_dir_all(&base)
...
fn get_or_create_macos_ramdisk() -> Result<PathBuf, String> {
  let mount_point = PathBuf::from("/Volumes/BugLoginEphemeral");
  ...
  let fmt = std::process::Command::new("diskutil")
    .args(["erasevolume", "HFS+", "BugLoginEphemeral", &dev])
    .output()
...
fn get_or_create_windows_ramdisk() -> Result<PathBuf, String> {
  ...
  for letter in ['R', 'Q', 'P', 'O'] {
    let base = PathBuf::from(format!("{}:\\BugLoginEphemeral", letter));
  ...
  for letter in ['R', 'Q', 'P', 'O'] {
    ...
    match output {
      Ok(out) if out.status.success() => {
        let base = PathBuf::from(format!("{}\\BugLoginEphemeral", drive));
        std::fs::create_dir_all(&base)
...
fn cleanup_legacy_dirs() {
  ...
  for entry in entries.flatten() {
    if let Some(name) = entry.file_name().to_str() {
      if name.starts_with("buglogin-ephemeral-") && entry.path().is_dir() {
